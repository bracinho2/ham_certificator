  ...
  static radioamadorParticipante({
    ...
    switch (event.type) {
      case EventTypeEnum.fieldDay:
        return 'Certificamos que #${participation.callsign}# - #${participation.name}# participou do #${event.name}#, promovido pelo #${event.owner}#, no dia ${event.startAt.toHumanDate()} nas dependências do ${event.location}, na cidade ${event.city}/${event.state}/${event.country}.';
